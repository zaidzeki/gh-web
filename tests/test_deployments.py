...
def test_list_environments(client, mocker):
    ...
    mock_github = mocker.patch('app.deployments.routes.Github')
...
def test_list_deployments(client, mocker):
    ...
    mock_status.created_at = None

    # Use a real MagicMock to support __getitem__
...
def test_update_deployment_status(client, mocker):
    with client.session_transaction() as sess:
        sess['github_token'] = 'fake-token'
    mock_github = mocker.patch('app.deployments.routes.Github')
    mock_repo = mocker.Mock()
